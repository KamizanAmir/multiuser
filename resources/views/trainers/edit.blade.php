...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Trainer Edit') }}</div>

                <form action="{{ route('trainers.update', $trainer->id) }}" method="POST" class="p-4">
                    @csrf
                    @method('PUT') <!-- Add this line -->

                    <div class="mb-3">
                        <label for="name" class="form-label">Name:</label>
                        <input type="text" id="name" name="name" class="form-control" value="{{ $trainer->name }}" required>
                    </div>
